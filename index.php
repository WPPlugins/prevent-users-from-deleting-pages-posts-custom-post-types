...
function highlight_disable(){
	...
	// *******************************************
	// Check if admin has ability to delete or not
	// *******************************************
	
		$ability = get_option('disable_admin');
		if($ability == 'enable' && current_user_can( 'manage_options' )){
																							 
			if(in_array($_GET['post_type'],$post_types)){
			
			// Check Color Scheme
			$current_user = get_current_user_id();
			$color_scheme = get_user_meta($current_user,'admin_color',true);
			...
			elseif($color_scheme == 'coffee'){
				$color = '#c7a589';
			}
			elseif($color_scheme == 'ectoplasm'){
				$color = '#a3b745';
			}
			elseif($color_scheme == 'midnight'){
				$color = '#e14d43';	
			}
			elseif($color_scheme == 'ocean'){
				$color = '#9ebaa0';
			}
			elseif($color_scheme == 'sunrise'){
				$color = '#dd823b';	
			}
			
			$disabled_pages = get_option('disable_pages');
			echo '<script>jQuery(document).ready(function(){';
			foreach($disabled_pages as $post){ ?>
					jQuery('#post-' + "<?php echo $post; ?>").css('background','<?php echo $color ?>');
					jQuery('#post-' + "<?php echo $post; ?>" + " a").css('color','#fff');
					jQuery('#post-' + "<?php echo $post; ?>" + " .column-date").css('color','#fff');
				<?php 
			}
			echo '});</script>';
		}
...
add_action( 'admin_footer', 'highlight_disable' );

/**********************/
/* Admin Notification */
/**********************/

function disable_delete_notice() {
	$post_types = get_post_types();
	...
	// *******************************************
		...
		if($ability == 'enable' && current_user_can( 'manage_options' )): 
			...
			echo '<div class="updated">
					<p><strong>Note:</strong> The Highlighted Rows Cannot Be Deleted !!!</p>
				</div>';
...
add_action( 'admin_notices', 'disable_delete_notice' );